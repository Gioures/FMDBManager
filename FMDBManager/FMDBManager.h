/*
 * z使用注意
 1、需要在模型中添加一个属性id，NSString类型，为了绑定对应的数据，从而进行增删改查操作。
 3、需要插入数据库的模型不支持继承，因为根据类名来创建表，框架内部只能读取当前类的属性，其父类的属性没办法获取。
 4、修改数据库中的模型数据只能通过指定的id作为条件修改。
 5、暂时不支持模型属性动态删减，如果删了对应属性（除了id）不影响使用，但如果增加属性了，只能从新建表存储。
...
#define DB_DEFAULT_NAME @"gioures"
